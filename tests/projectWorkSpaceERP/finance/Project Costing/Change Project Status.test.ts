...
test("Create a Project From Template", async ({ page }) => {
  ...
  await commonFunctions.selectTastkFromTasksPanel("Initiate", "Manage Financial Project Settings");
  await createProject.ProjectClassifications(firstRecord);
  await createProject.ChangeProjectStatus(firstRecord);
