...
const keywords = {
    ...
    // ===== CLICK KEYWORDS =====
    Web_ClickByText: async (page, params) => {
    ...
    Web_ClickByAltOrTitle: async (page, params) => {
        await waitForAppReady(page);
        const { label, partial = false, index = 0 } = params;
        try {
            let locator;
    ...
    Web_ClickByXPath: async (page, params) => {
        await waitForAppReady(page);
    ...
    // ===== INPUT/TYPE KEYWORDS =====
    Web_TypeByText: async (page, params) => {
        await waitForAppReady(page);
    ...
    Web_SelectDropdownByText: async (page, params) => {
        await waitForAppReady(page);
        const { label, value, partial = false, index = 0 } = params;
        try {
            const selectorParts = [
    ...
    Web_SelectDropdownByIndex: async (page, params) => {
        await waitForAppReady(page);
    ...
    // ===== VALIDATION KEYWORDS =====
    isTextPresentOnScreen: async (page, params) => {
        await waitForAppReady(page);
        const { label, partial = false, index = 0 } = params;
        try {
            const element = page.getByText(label, { exact: !partial }).nth(index);
    ...
    "Verify Text": async (page, params) => {
        const element = page.locator(params.selector);
        await (0, test_1.expect)(element).toHaveText(params.expectedText);
        logStep(`✅ Verified text: ${params.expectedText}`, "Verify Text");
        return true;
    },
    "Verify Visible": async (page, params) => {
        const element = page.locator(params.selector);
        await (0, test_1.expect)(element).toBeVisible();
        logStep(`✅ Verified visible: ${params.selector}`, "Verify Visible");
        return true;
    },
    "Verify Title": async (page, params) => {
        await (0, test_1.expect)(page).toHaveTitle(new RegExp(params.title));
        logStep(`✅ Verified title: ${params.title}`, "Verify Title");
        return true;
    },
    // ===== TABLE KEYWORDS =====
    Web_TypeTextInTableCell: async (page, params) => {
        await waitForAppReady(page);
        ...
        try {
            ...
            const cell = tableLocator.locator(`tbody tr:nth-child(${rowNumber}) td:nth-child(${colIndex + 1})`);
    ...
    "Wait For Selector": async (page, params) => {
