...
export const keywords = {
  ...
  'OpenBrowser': async (page: Page, params: KeywordParams) => {
  ...
  'Web_ClickByText': async (page: Page, params: KeywordParams) => {
    ...
    // Fallback 1: SVG title
    try {
      const svgClicked = await page.evaluate((labelText) => {
        const titles = Array.from(document.querySelectorAll('svg title'));
        const match = titles.find((title) => title.textContent?.trim().includes(labelText));
        if (match) {
          const clickable = match.closest('a, button, span, div') as HTMLElement;
          if (clickable) {
            clickable.setAttribute('data-temp-click', 'true');
            clickable.click();
            return true;
          }
        }
      ...
      if (svgClicked) {
        const tempLocator = page.locator('[data-temp-click="true"]');
        await tempLocator.waitFor({ state: 'attached', timeout: DEFAULT_TIMEOUT });
        await highlight(tempLocator);
        await page.evaluate(() => {
          const el = document.querySelector('[data-temp-click="true"]');
          if (el) el.removeAttribute('data-temp-click');
        });
        logStep(`✅ Clicked '${label}' via SVG/title fallback`, 'Web_ClickByText');
        return true;
      }
    } catch (svgError) {
      console.warn(`⚠️ SVG/title fallback failed`);
    }

    // Fallback 2: Attributes
    try {
      const attrLocator = page
        .locator(`[title="${label}"], [alt="${label}"], [aria-label="${label}"]`)
        .first();
      await attrLocator.waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });
      await highlight(attrLocator);
      await attrLocator.click();
      logStep(`✅ Clicked '${label}' via attribute selector`, 'Web_ClickByText');
      return true;
    } catch (attrError) {
      console.warn(`⚠️ Attribute fallback failed`);
    }

    // Fallback 3: DOM text search
    try {
      const clickedFallback = await page.evaluate((labelText) => {
        const elements = Array.from(document.querySelectorAll('span, div, a, td, button'));
        const match = elements.find((el) => el.textContent?.trim().includes(labelText));
        if (match) {
          (match as HTMLElement).setAttribute('data-temp-fallback', 'true');
      ...
      if (clickedFallback) {
        const fallbackLocator = page.locator('[data-temp-fallback="true"]');
        await fallbackLocator.waitFor({ state: 'attached', timeout: DEFAULT_TIMEOUT });
        await highlight(fallbackLocator);
        await page.evaluate(() => {
          const el = document.querySelector('[data-temp-fallback="true"]');
          if (el) el.removeAttribute('data-temp-fallback');
        });
        logStep(`✅ Clicked '${label}' via fallback DOM match`, 'Web_ClickByText');
        return true;
      }
    } catch (finalError) {
      console.warn(`⚠️ Fallback DOM match failed`);
    }
  ...
  'Web_ClickByAltOrTitle': async (page: Page, params: KeywordParams) => {
    ...
    try {
      ...
      await target.click();
      logStep(`✅ Clicked element with alt/title: "${label}"`, 'Web_ClickByAltOrTitle');
      return true;
    } catch (error) {
  ...
  'Web_ClickByXPath': async (page: Page, params: KeywordParams) => {
    ...
    try {
      ...
      console.error(`❌ Failed to click XPath: ${xpath}. Reason: ${error.message}`);
  ...
  'Web_TypeByText': async (page: Page, params: KeywordParams) => {
    await waitForAppReady(page);
    ...
    try {
      ...
      try {
        const inputs = page
          .locator(`input[placeholder*="${label}"], textarea[placeholder*="${label}"]`)
          .nth(index);
        await inputs.waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });
        await highlight(inputs);
        await inputs.fill(value);
        logStep(`✅ Typed '${value}' in placeholder '${label}'`, 'Web_TypeByText');
        return true;
      } catch (placeholderError: any) {
        console.error(`❌ Failed to type in '${label}': ${placeholderError.message}`);
        logStep(`❌ Field not found for label/placeholder: ${label}`, 'Web_TypeByText');
        return false;
      }
    }
  },
  ...
  'Web_SelectDropdownByText': async (page: Page, params: KeywordParams) => {
    ...
    try {
      ...
      if (tagName === 'select') {
        ...
        await page.getByRole('option', { name: value, exact: !partial }).click();
      }
      ...
      try {
        ...
        logStep(`✅ Selected '${value}' in fallback '${label}'`, 'Web_SelectDropdownByText');
        return true;
      } catch (fallbackError: any) {
        console.error(`❌ Failed to select in '${label}': ${fallbackError.message}`);
        logStep(`❌ Dropdown not found for label: ${label}`, 'Web_SelectDropdownByText');
        return false;
      }
    }
  },
  ...
  'Web_SelectDropdownByIndex': async (page: Page, params: KeywordParams) => {
    ...
    try {
      ...
      logStep(`✅ Selected index ${optionIndex} from '${label}'`, 'Web_SelectDropdownByIndex');
      return true;
    } catch (err) {
    ...
    const xpathLocator = page
      .locator(`//label[normalize-space(text())="${label}"]/following::select[1]`)
      .nth(index);
    try {
      await xpathLocator.waitFor({ state: 'visible', timeout: DEFAULT_TIMEOUT });
      await highlight(xpathLocator);
      await xpathLocator.selectOption({ index: optionIndex });
      logStep(`✅ Selected index ${optionIndex} via XPath for label '${label}'`, 'Web_SelectDropdownByIndex');
      return true;
    } catch (err) {
      throw new Error(`❌ Could not select index ${optionIndex} from dropdown '${label}'`);
    }
  },
  ...
  'isTextPresentOnScreen': async (page: Page, params: KeywordParams) => {
    ...
    try {
      ...
      if (isVisible) {
        ...
        logStep(`❌ Text '${label}' is not found on the screen`, 'isTextPresentOnScreen');
        return false;
      }
    } catch (error: any) {
      console.error(`⚠️ Error while verifying text '${label}': ${error.message}`);
  ...
  'Web_TypeTextInTableCell': async (page: Page, params: KeywordParams) => {
    await waitForAppReady(page);
    const { tableSelector, columnName, rowNumber, value, partial = false } = params;
    ...
    try {
      ...
      if (colIndex === -1 && partial) {
        colIndex = headers.findIndex((h) => h.toLowerCase().includes(columnName.toLowerCase()));
      }

      if (colIndex === -1) {
        console.error(`❌ Column "${columnName}" not found`);
        logStep(`❌ Column "${columnName}" not found`, 'Web_TypeTextInTableCell');
        return false;
      }
      ...
      if ((await inputField.count()) > 0) {
        ...
        logStep(`✅ Typed '${value}' into ${columnName} (Row ${rowNumber})`, 'Web_TypeTextInTableCell');
      } else {
        await cell.click({ clickCount: 2 });
        await page.keyboard.type(value);
        logStep(`✅ Typed '${value}' into ${columnName} (Row ${rowNumber}) - direct edit`, 'Web_TypeTextInTableCell');
      }

      return true;
    } catch (error: any) {
      console.error(`❌ Failed to type in table cell: ${error.message}`);
      logStep(`❌ Failed to type '${value}' in ${columnName} (Row ${rowNumber})`, 'Web_TypeTextInTableCell');
      return false;
    }
  },
  ...
  'Wait For Selector': async (page: Page, params: KeywordParams) => {
    await page.waitForSelector(params.selector, { timeout: params.timeout || DEFAULT_TIMEOUT });
    logStep(`Waited for selector: ${params.selector}`, 'Wait For Selector');
  },
  
  // ===== PAGE OBJECT KEYWORDS (Legacy) =====
  
  'Open Home Page': async (page: Page) => {
    const home = new HomePage(page);
    await home.goto();
    logStep('Opened Home Page', 'Open Home Page');
  },
  
  'Click Get Started': async (page: Page) => {
    const home = new HomePage(page);
    await home.getStarted();
    logStep('Clicked Get Started', 'Click Get Started');
  },
  
  'Verify Installation Heading': async (page: Page) => {
    const home = new HomePage(page);
    const result = await home.isInstallationHeadingVisible();
    logStep(`Installation heading visible: ${result}`, 'Verify Installation Heading');
    return result;
  },
  
  'Create Invoice': async (page: Page, params: KeywordParams) => {
    ...
    logStep('Created invoice', 'Create Invoice');
  },
  
  'Verify Invoice Created': async (page: Page) => {
    const invoicePage = new InvoicePage(page);
    const result = await invoicePage.isInvoiceCreated();
    logStep(`Invoice created: ${result}`, 'Verify Invoice Created');
    return result;
  },
};
